...
"Para revisar si en nuesto DataFrame existen datos faltanten podemos usar 
...
# Aquí los argumentos de las funciones de vector se alimentan: primero filas y luego columnas
...
cual nos permite pasar una función específica sobre los márgenes (1: filas o 2: columnas) 
de una matriz o DataFrame:"

set.seed(2) #Esto declara el valor de semilla para los valores aleatorios
...
visitas.rest.select <- select(visitas.rest, date, spending:rest3)#
...
visitas.rest.pipe <- visitas.rest %>% select(date, spending, total)

class(visitas.rest.pipe)
...
visitas.rest.per.visita <- visitas.rest %>%
                              select(date, spending, total) %>%
                                  mutate(spending.per.visita = spending/total) %>%
                                      arrange (desc(spending.per.visita))
...
"Nota: la función «factor» convierte una variable de tipo cuantitativa a cualitativa"
